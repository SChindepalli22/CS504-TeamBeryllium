...
##Linear modeling with grad_debt_mdn10yr
insDataF <- insData[ ,c(1:72, 80,84)]
trainingRows <- createDataPartition(y = insDataF$grad_debt_mdn10yr, p = .8,list= FALSE)
...
set.seed(622)
plsModelI2 <- train(grad_debt_mdn10yr ~., data = trainData, method = "pls",
                    trControl = trainControl(method = "repeatedcv", repeats = 5))
plsModelI2

plot(varImp(plsModelI2))  

plsPredPLS <- predict(plsModel, testData) 
lmValuesPLS = data.frame(obs = testData$earn_count_wne_hi_1yr, pred = plsPredPLS)
defaultSummary(lmValuesPLS) 

#Linear Model - 6 - play around 
set.seed(622)
lmModelI6 <- train(grad_debt_mdn10yr ~ opeid6, data = trainData, method = "lm", 
                   trControl = trainControl(method = "LGOCV"))
lmModelI6


lmPredictI6 <- predict(lmModelI6, testData) 
lmValuesI6 = data.frame(obs = testData$debt_mdn, pred = lmPredictI6)
#View(lmValues)
defaultSummary(lmValuesI6)


----------------------------------------------------

#NonLinear nearest neighbor
 
set.seed(622)
knnModel1 = train(debt_all_stgp_any_mdn ~., data = trainData, method="knn")
knnModel1


knnPred = as.data.frame(predict(knnModel1, newdata=testData))
knn1PR = postResample(pred=knnPred, obs=testData$DEBT_ALL_STGP_ANY_MEAN)
knn1PR

 
 set.seed(622)
 knnModel2 = train(DEBT_ALL_STGP_ANY_MEAN ~ EARN_MDN_HI_2YR,data = trainData, method="knn",
 ...
 knnPred2 = as.data.frame(predict(knnModel2, newdata=testData))
 knn2PR = postResample(pred=knnPred2, obs=testData$DEBT_ALL_STGP_ANY_MEAN)
 knn2PR
 
 
 
 set.seed(622)
 knnModel3 = train(DEBT_ALL_STGP_ANY_MEAN ~ EARN_MDN_HI_1YR,data = trainData, method="knn",
                   preProc=c("BoxCox", "center", "scale"),
                   tuneLength=10,
                   trControl = trainControl(method = "repeatedcv", repeats = 5))
 knnModel3
 ...
 knnPred3 = as.data.frame(predict(knnModel3, newdata=testData))
 knn3PR = postResample(pred=knnPred3, obs=testData$DEBT_ALL_STGP_ANY_MEAN)
 knn3PR
 ...
 set.seed(622)
 knnModel4 = train(DEBT_ALL_STGP_ANY_MEAN ~ BBRR2_FED_COMP_DFLT,data = trainData, method="knn",
                   preProc=c("BoxCox", "center", "scale"),
                   tuneLength=10,
                   trControl = trainControl(method = "repeatedcv", repeats = 5))
 knnModel4
 ...
 knnPred4 = as.data.frame(predict(knnModel4, newdata=testData))

 knn4PR = postResample(pred=knnPred4, obs=testData$DEBT_ALL_STGP_ANY_MEAN)
 knn4view = as.data.frame(x=knnPred4, obs=testData$DEBT_ALL_STGP_ANY_MEAN)
 View(knn4view)
 knn4PR
 
knnPred4 <- predict(knnModel4, newdata=testData) 
knnValues4 = data.frame(obs = testData$DEBT_ALL_STGP_ANY_MEAN, pred = knnPred4)
View(knnValues4)
 
par(mfrow=c(2, 2))
plot(ldaClassVal)
plot(plsClassPred)
plot(plsClassTunedPred)